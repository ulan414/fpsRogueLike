...
namespace InfimaGames.LowPolyShooterPack
{
	...
	public sealed class Character : CharacterBehaviour
	{
		...
		protected override void Update()
		{
            ...
            {
				if(equippedWeapon.GetAmmunitionCurrent() == 0 && CanPlayAnimationReload() && equippedWeapon.GetAmmunitionTotal() != 0)
                {
					//reload if current ammo is 0
					PlayReloadAnimation();
				}
            }
		...
		private bool CanPlayAnimationReload()
		{
			...
			if (inspecting)
				return false;
			//block if magazine is full
			if (equippedWeapon.GetAmmunitionCurrent() == 30)
				return false;
